...
Anyone can override the automatic level at any time. Staff can do it from the Heater Control page, and guests can do it from the QR code on the pendant. A manual change always wins.
...
**Run outside your hours once.** From the dashboard, switch to Manual Mode, run the heaters, then switch back to Schedule Mode. Your saved hours stay untouched. Don't leave the system in Manual Mode overnight.
...
Outside your scheduled hours, heaters can't be controlled by the app or the pull-string until you adjust the schedule or switch to Manual Mode. During those times a heater shows as disabled, its purple and orange lights are off, and it flashes yellow if someone pulls the string.
