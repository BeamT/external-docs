---
title: Survey Your Patio
audience: [customer]
audience_order: {customer: 5}
order: 3
summary: Scan, photograph, and measure your patio for the Focal Site Survey.
version: "1.0"
updated: "2026-09-24"
---

## On this page
- [Why we ask](#why-we-ask)
- [What you'll need](#what-youll-need)
- [The order to do it in](#the-order-to-do-it-in)
- [Scan your patio](#scan-your-patio)
- [Wait for processing](#wait-for-processing)
- [Take the photos](#take-the-photos)
- [Mark what you'll measure](#mark-what-youll-measure)
- [Measure](#measure)
- [Share the Polycam link](#share-the-polycam-link)
- [Sketch, if you need one](#sketch-if-you-need-one)
- [Tables not set up yet?](#tables-not-set-up-yet)
- [Can't get a scan?](#cant-get-a-scan)
- [Fill out the form](#fill-out-the-form)
- [What happens next](#what-happens-next)

## Why we ask

Focal uses your scan, photos, and measurements to draw a Preliminary Layout: where the rails and heaters go, and what your electrician needs to know. You do the survey, so we don't need to visit your site first.

## What you'll need

{% include checklist.html items="An iPhone 13 or newer with the free Polycam app and a free Polycam account, for the scan. It can be anyone's on your staff | Any phone, for the photos and the form | A tape measure, 25 ft or longer if you have one. A second person to hold the other end helps | Tables and chairs set up the way they'll be during service. We count and place them from your scan (not set up yet? See below) | About 15 minutes for the form, plus the scan and its processing time | Pen and paper, only if the covered area isn't a rectangle (see Sketch, below)" %}

## The order to do it in

Do everything in this guide **before** you open the form. The form asks for each piece in turn, so it goes faster when they're ready.

1. Scan your patio in Polycam on an iPhone and let it process.
2. Take the photos.
3. Mark up the corner photo.
4. Measure.
5. Draw a sketch, if the covered area isn't a rectangle.
6. Open the [Focal Site Survey form](https://form.typeform.com/to/DQXI87AN), answer the questions, upload the photos and paste the Polycam link.

## Scan your patio

The scan needs an **iPhone 13 or newer** (Polycam's Space mode isn't on Android). Anyone on your staff can do it and send you the link. [Get Polycam free from the App Store](https://apps.apple.com/us/app/polycam-3d-scanner-lidar-360/id1532482376), then open it and create a free account.

Scan the whole covered area in one go. Polycam may not warn you about a missed spot, so use this list as your check:

{% include checklist.html items="The whole covered area, end to end | The cover itself: the awning, canopy, roof, or beams. Point your phone up as well as around | Walls, posts, and planters | Tables and chairs, and their edges | Where the building meets the covered area" %}

### Scan steps

{% include step.html number="1" title="Start a new capture" body="Open Polycam, start a new capture, and choose <strong>Space</strong>. Turn on the floor plan option if you see one." %}

{% include step.html number="2" title="Start at a corner" body="Stand in one corner of the covered area, next to the building if there is one. Hold your phone upright at chest height and start the capture." %}

{% include step.html number="3" title="Walk slowly around the edge, facing in" body="Walk the edge of the covered area at a slow, steady pace, <strong>facing into the area</strong>. Don't stand in one spot and turn: the app needs views from different spots to build the model." %}

{% include step.html number="4" title="Get the cover and the floor" body="As you go, tilt the phone up to catch the awning, canopy, or beams, then down to the floor edges." %}

{% include step.html number="5" title="Close the loop" body="Keep going to the far end and back to your starting corner, then finish the capture. Don't stop before the far end." %}

### Common mistakes

{% include dodont.html do="Move slowly and steadily, and point up at the cover as well as around." dont="Rush, skip the cover or ceiling, or stop before the far end of the area." %}

## Wait for processing

After you finish, Polycam processes the scan. If it asks where to process, choose **Cloud**, so you can share the link. **This can take a while.** Keep the app open and your phone on WiFi if you can.

- It's done when the scan opens as a 3D model you can spin around.
- Don't copy the share link until then.
- While you wait, take the photos and measurements below.

## Take the photos

You'll take three photos. Use your phone's normal camera, not Polycam.

| Photo | How to take it |
|---|---|
| **Front view** | Stand back far enough to fit the whole seating area in one shot, facing it straight on (usually from the street or the entrance). Include the ceiling or awning. **Don't draw on this one**: it goes on the cover of your layout |
| **Side view** | Stand at one end and shoot along the long side, so we can see the height and what's overhead |
| **Corner photo** | One more photo from a corner, showing the front and side of the covered area. You'll draw on this one next |

{% include diagram.html src="survey-front-view.svg" fill=true caption="Front view: the whole seating area, straight on, cover included." %}

{% include diagram.html src="survey-side-view.svg" fill=true caption="Side view: along the long side, showing the height and what's overhead." %}

## Mark what you'll measure

Draw three lines on the corner photo and label them **W**, **D**, and **H**. This photo is your measuring plan: each number you enter in the form matches a line you drew, so width and depth don't get mixed up.

| Line | From | To |
|---|---|---|
| **W** (width) | One end of the awning, canopy, or roof | The other end |
| **D** (depth) | The building wall (or the back edge of the cover) | The front edge of the cover |
| **H** (height) | The floor | The lowest beam or awning frame |

**Draw to things you can see**, like a beam or an awning edge, not a spot on the ground.

To draw on the photo:
- **iPhone:** Photos → Edit → Markup
- **Android:** Google Photos → Edit → Markup

{% include diagram.html src="survey-corner-markup.svg" fill=true caption="Corner photo with W, D, and H lines drawn to the edges of the cover." %}

{% include warn.html text="Draw only on the corner photo. Keep the front and side photos clean." %}

## Measure

Measure each line you drew on the corner photo: W, D, and H.

- Measure the **covered area**, out to the edges of the cover. Don't measure just where the tables sit.
- Feet and inches are fine, e.g. *18 ft 6 in*. No decimals needed.
- Write the three numbers down, or keep them on the corner photo.

## Share the Polycam link

Once processing has finished:

{% include step.html number="1" title="Open your scan" body="In Polycam, open the scan of your patio." %}

{% include step.html number="2" title="Allow anyone with the link to view it" body="Tap <strong>Share</strong>. Make sure the scan is shared by link, so <strong>anyone with the link</strong> can view it. We open it from a Focal account, so an invite-only share won't reach us." %}

{% include step.html number="3" title="Copy the link" body="Tap <strong>Copy link</strong>, then paste it into the form's Polycam question. If someone else is filling out the form, text or email them the link." %}

{% include warn.html text="Wait until the scan has finished processing before you copy the link." %}

## Sketch, if you need one

You only need a sketch if **the covered area isn't a rectangle**. L-shapes, angled sides, or more than 4 sides count. Draw the shape, leave a gap for the entrance, and **write the length on every side**. Measure each side while the tape is out. A photo of a hand drawing on paper is fine.

{% include diagram.html src="survey-shape-rect-vs-l.svg" caption="A rectangle needs no sketch. An L-shape (or any other shape) does." %}

## Tables not set up yet?

If your patio is new or the furniture is put away, do the scan, photos, and measurements anyway. Then **reply to the email your survey link came in**, and a Focal team member will set up a video call to plan where the tables go.

## Can't get a scan?

If no one on your staff has an iPhone that can scan, the scan won't work, or we find it doesn't show enough, a Focal team member will set up a guided video call and walk through your patio with you.

To ask for one, **reply to the email your survey link came in.**

## Fill out the form

When your photos, measurements, and Polycam link are ready, **[open the Focal Site Survey form](https://form.typeform.com/to/DQXI87AN)**.

## What happens next

We'll email your Preliminary Layout within 2 business days of your form. If anything is missing from the scan or photos, we'll let you know.

Questions? Reply to the email your survey link came in.
